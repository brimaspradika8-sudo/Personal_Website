...
export default function AdminArticlesClient({ initialArticles }: ArticlesClientProps) {
  ...
  const handleDelete = (id: string, title: string) => {
    ...
    setConfirmModalState({
      ...
      description: `Are you sure you want to delete the article "${title}"? This action is permanent and cannot be undone.`,
      isLoading: false,
      onConfirm: async () => {
        setConfirmModalState((prev) => ({ ...prev, isLoading: true }));
        ...
        <header className={`h-16 shrink-0 flex items-center justify-between px-4 sm:px-6 z-10 sticky top-0 backdrop-blur-md border-b transition-colors duration-300 ${isNight ? "bg-[#0B0F17]/80 border-slate-800" : "bg-white/90 border-slate-200 shadow-xs"}`}>
          ...
          <div className="flex items-center gap-2.5 ">
            ...
            >
              ...
              <span className="hidden sm:inline">Comment Moderation</span>
            </button>
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between ">
              ...
              <div className="text-xs text-slate-600 dark:text-slate-400 font-medium">
                Total Articles: <span className="font-extrabold text-slate-900 dark:text-slate-100">{filteredArticles.length}</span>
              </div>
            </div>
          ...
          <div className={`w-full max-w-2xl rounded-3xl border p-6 space-y-6 shadow-2xl ${isNight ? "bg-[#0E1015] border-slate-800 text-slate-100" : "bg-white border-slate-200 text-slate-900"}`}>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
              {loadingComments ? (
                ...
                <div className="py-8 text-center text-xs text-slate-500">No user comments yet.</div>
              ) : (
        ...
        <div className="fixed inset-0 z-50 bg-[#F8F9FA] text-slate-900 overflow-y-auto p-4 sm:p-6 lg:p-10 space-y-8">
          ...
          <div className="flex items-center justify-between border-b border-slate-200 pb-4 max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-3">
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-xs text-amber-800 font-medium">
                ...
                <span>{editingArticle ? "Editing article" : "New draft (not published yet)"}</span>
              </div>
              <button
                type="button"
                onClick={() => setIsModalOpen(false)}
          ...
          <div className="space-y-2 max-w-7xl mx-auto">
            <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
              {editingArticle ? "Edit Article" : "Write a New Article"}
            </h1>
            <p className="text-xs sm:text-sm text-slate-600 max-w-2xl leading-relaxed">
              Capture your ideas, document new insights, and share technical experiments with clarity.
            </p>
          </div>
                ...
                <div className="space-y-2 pt-2">
                  <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden p-1 shadow-sm">
                    <RichTextEditor
                      content={content}
                      onChange={setContent}
                      placeholder="Write your article content and insert images here..."
                    />
                  </div>
                </div>
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                <div className="p-5 sm:p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-xs">
                  ...
                  <p className="text-[11px] text-slate-500 leading-relaxed pt-1">
                    The cover image will appear in the article header, journal card, and OpenGraph preview.
                  </p>
                </div>
                ...
                <div className="p-5 sm:p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-xs">
                  ...
                  <div className="space-y-3 text-xs">
                    ...
                    <div className="flex items-center justify-between py-1">
                      ...
                      <span className="font-medium text-slate-900">Article / Journal</span>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-100 pt-4 space-y-2">
                    ...
                    >
                      ...
                      <span>{loading ? "Saving..." : editingArticle ? "Save Changes" : "Publish Article"}</span>
                    </button>
