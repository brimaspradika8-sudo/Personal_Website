...
export default function EditArtikelClient({ article }: EditArtikelClientProps) {
  ...
  const handleThumbnailUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if ("error" in res && res.error) {
      ...
      setStatusMsg({ type: "success", text: "Cover image updated in Supabase Storage!" });
    }
  ...
  const debouncedSubmit = useDebouncedAction(async () => {
    ...
    if (res.error) {
      ...
      setStatusMsg({ type: "success", text: "Article changes saved successfully!" });
      setTimeout(() => {
