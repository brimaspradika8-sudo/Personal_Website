...
export default function TambahArtikelClient() {
  ...
  const handleRestoreDraft = () => {
    ...
    try {
      ...
      if (saved) {
        ...
        setStatusMsg({ type: "success", text: "Draft restored successfully!" });
      }
    } catch {}
    setHasDraft(false);
  };
  ...
  const handleThumbnailUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if ("error" in res && res.error) {
      ...
      setStatusMsg({ type: "success", text: "Cover image uploaded to Supabase Storage!" });
    }
  ...
  const debouncedSubmit = useDebouncedAction(async () => {
    ...
    if (!title.trim() || !slug.trim() || !content.trim()) {
      setStatusMsg({ type: "error", text: "Article title, slug, and content are required." });
      return;
    }
    ...
    if (res.error) {
      ...
      localStorage.removeItem(DRAFT_KEY);
      setStatusMsg({ type: "success", text: "New article published successfully!" });
      setTimeout(() => {
      ...
      <header className="h-16 border-b-4 border-black dark:border-white px-4 sm:px-8 flex items-center justify-between sticky top-0 bg-white/95 dark:bg-[#0E131F]/95 backdrop-blur-md z-30">
        <div className="flex items-center gap-3">
          <Link
            ...
            title="Back to article studio"
          >
            <ArrowLeft className="w-4 h-4" />
            <span className="hidden sm:inline">Article Studio</span>
          </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>{loading ? "Publishing..." : "Publish (Ctrl+S)"}</span>
          </button>
        </div>
      </header>
        ...
        {hasDraft && (
          <div className="p-4 bg-[#FEF08A] text-slate-950 border-4 border-black text-xs font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span>A previous draft was detected in this browser.</span>
            </div>
                ...
                <div className="flex items-center justify-between border-b-2 border-black dark:border-white pb-2">
                  ...
                  <div className="flex items-center gap-4 text-[11px] font-bold text-slate-600 dark:text-slate-400">
                    <span className="flex items-center gap-1">
                      <FileText className="w-3.5 h-3.5 text-[#166534] dark:text-[#00FF66]" /> {wordCount} words
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5 text-[#EAB308]" /> ~{readTimeMinutes} min read
                    </span>
                  </div>
                </div>
