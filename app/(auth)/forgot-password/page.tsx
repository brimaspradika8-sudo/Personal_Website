...
export default function ForgotPasswordPage() {
  ...
  const debouncedSendOtp = useDebouncedAction(async () => {
    ...
    try {
      ...
      if (res.error) {
        ...
        setSuccessMsg(`A 6-DIGIT OTP CODE HAS BEEN SENT TO: ${email.toUpperCase()}`);
        setStep(2);
        setResendTimer(30);
      }
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "An error occurred while sending the OTP.");
    } finally {
      setLoading(false);
    }
  }, 700);
  ...
  const debouncedVerifyOtp = useDebouncedAction(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "An error occurred while verifying the OTP.");
    } finally {
      setLoading(false);
    }
  }, 700);
