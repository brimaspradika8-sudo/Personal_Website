import React from "react";

export default function PricingLoading() {
  return (
    <div className="min-h-screen bg-white dark:bg-black text-black dark:text-white font-mono selection:bg-[#EAB308] selection:text-black pb-28 sm:pb-20">
      {/* Sticky Header Skeleton */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 dark:bg-black/95 backdrop-blur-md border-b-4 border-black dark:border-white px-4 sm:px-8 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 animate-pulse">
          <div className="flex items-center gap-3 shrink-0">
            <div className="w-9 h-9 rounded-none bg-[#EAB308] border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" />
            <div className="w-32 sm:w-40 h-5 rounded-none bg-black dark:bg-white" />
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <div className="w-24 h-9 rounded-none bg-[#16A34A] border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" />
          </div>
        </div>
      </header>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-28 space-y-12 animate-pulse">
        {/* Title Header Skeleton */}
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          <div className="w-48 h-8 rounded-none bg-[#EAB308] border-2 border-black mx-auto shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]" />
          <div className="w-3/4 h-12 rounded-none bg-neutral-300 dark:bg-neutral-800 border-2 border-black dark:border-white mx-auto" />
          <div className="w-1/2 h-5 rounded-none bg-neutral-200 dark:bg-neutral-900 mx-auto" />
        </div>

        {/* Pricing Cards Grid Skeleton */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="h-[460px] rounded-none border-4 border-black dark:border-white bg-neutral-100 dark:bg-neutral-900 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]" />
          <div className="h-[500px] rounded-none border-4 border-black dark:border-white bg-[#FEF9C3] dark:bg-[#182015] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]" />
          <div className="h-[460px] rounded-none border-4 border-black dark:border-white bg-neutral-100 dark:bg-neutral-900 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]" />
        </div>
      </div>
    </div>
  );
}
