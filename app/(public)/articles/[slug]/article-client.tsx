...
const READING_LABEL = "ARTICLE SECTION IN PROGRESS";
...
const buildLineRanges = (blocks: ReturnType<typeof parseContentBlocks>) => {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title={isSavedBookmark ? "Saved" : "Save article"}
          >
      ...
      <div className="flex items-center gap-3.5 pt-3 pb-5 border-b-4 border-black dark:border-white">
        ...
        <div className="space-y-0.5">
          <p className="font-black text-sm sm:text-base uppercase text-black dark:text-white leading-tight">
            {article.authorName || "Platform Author"}
          </p>
          <p className="text-xs font-bold text-neutral-600 dark:text-neutral-400 uppercase">
            {article.authorName?.toLowerCase().includes("brimas")
              ...
              : "Platform Author · Article Studio"}
          </p>
        </div>
      </div>
    </div>
...
function ArticleTocSidebar({
  ...
  setFontSizeScale: (value: "sm" | "base" | "lg" | "xl") => void;
}) {
  return (
        ...
        <div className={`${toc.length > 0 ? "pt-3 border-t-3 border-black dark:border-white" : ""} space-y-2 `}>
          <div className="flex items-center justify-between text-xs font-black uppercase text-black dark:text-white">
            ...
            <span className="text-[10px] text-[#166534] dark:text-[#EAB308]">
              {fontSizeScale === "sm" ? "SMALL" : fontSizeScale === "base" ? "NORMAL" : fontSizeScale === "lg" ? "LARGE" : "XL"}
            </span>
          </div>
          <div className="grid grid-cols-4 gap-1 bg-neutral-100 dark:bg-neutral-900 p-1.5 border-3 border-black dark:border-white">
            {(["sm", "base", "lg", "xl"] as const).map((sz) => (
              <button
                ...
                onClick={() => {
                  setFontSizeScale(sz);
                  showToast(`Font size changed to: ${sz === "sm" ? "Small" : sz === "base" ? "Normal" : sz === "lg" ? "Large" : "Extra Large"}`);
                }}
...
function ArticleContent({
  ...
  parsedBlocks: ReturnType<typeof parseContentBlocks>;
  renderParsedBlocks: (blocks: ReturnType<typeof parseContentBlocks>) => React.ReactNode;
}) {
  return (
...
function ArticleComments({
  ...
  onDeleteCommentConfirm: () => void;
  deleteCommentId: string | null;
  setDeleteCommentId: (value: string | null) => void;
}) {
  return (
    <section className="space-y-6 pt-4 ">
      ...
      {user ? (
        <form onSubmit={onCommentSubmit} className="p-4 sm:p-5 rounded-2xl border-2 border-black dark:border-white bg-white dark:bg-black space-y-4 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:shadow-[3px_3px_0px_0px_rgba(255,255,255,1)]">
          <div className="flex items-center gap-2 text-xs font-bold text-black dark:text-white">
            ...
            <span>{replyTargetId ? "REPLYING TO COMMENT" : "WRITING AS"} <strong className="text-[#166534] dark:text-[#EAB308] uppercase">{user.user_metadata?.full_name || user.email?.split("@")[0]}</strong></span>
          </div>

          <textarea
            rows={3}
            placeholder={replyTargetId ? "Write your reply..." : "Write your perspective..."}
            value={commentText}
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-[#FFFF00] text-black border-4 border-black text-xs font-black flex items-center justify-between shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
          <Link
            href={`/login?message=${encodeURIComponent("Please log in first to comment")}`}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 pt-1 text-[10px] font-black uppercase">
                    <button type="button" aria-label={`Sukai komentar ${comment.user.name}`} onClick={() => onLikeComment(comment.id)} className={`inline-flex items-center gap-1 min-h-10 ${comment.likedByUser ? "text-[#166534]" : "text-neutral-500"}`}>
      ...
      <ConfirmModal
        ...
        onConfirm={onDeleteCommentConfirm}
...
export default function ArticleClient({
  ...
  useEffect(() => {
    if (!toastMsg) return;
    const timeoutId = window.setTimeout(() => setToastMsg(null), 3000);
        ...
        {blocks.map((b) => {
          ...
          if (b.tag === "h2") {
            return (
              ...
              >
                {isActiveReading && renderReadingBadge("px-2.5 py-0.5 mb-1.5")}
          ...
          if (b.tag === "li") {
            return (
              ...
              >
                {isActiveReading && renderReadingBadge("px-2 py-0.5 mb-1 mr-2")}
          ...
          if (b.tag === "blockquote") {
            return (
              ...
              >
                {isActiveReading && renderReadingBadge("px-2.5 py-0.5 mb-1.5 not-italic")}
                <span dangerouslySetInnerHTML={{ __html: b.content }} />
              </blockquote>
            );
          }
  ...
  const handleCopyCode = useCallback((codeText: string, index: number) => {
    if (navigator.clipboard) {
      navigator.clipboard.writeText(codeText);
      setCopiedCodeIndex(index);
      showToast("Kode berhasil disalin!");
      window.setTimeout(() => setCopiedCodeIndex(null), 2000);
    }
  }, [showToast]);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <ArticleContent
            ...
            renderParsedBlocks={renderParsedBlocks}
          />
        </div>
        ...
        {relatedArticles.length > 0 && (
          <section className="space-y-4 pt-8 border-t-4 border-black dark:border-white ">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {relatedArticles.slice(0, 2).map((rel: ArticleItem) => (
                <Link
                  key={rel.id}
