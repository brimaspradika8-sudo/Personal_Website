...
export default function ArticleClient({
  ...
  const playElevenLabsAudio = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const userMsg = errorData.error || "Failed to process Edge Neural AI.";
        showToast(`${userMsg} Falling back to browser voice...`);
        startSpeech(audioSpeed);
        setIsLoadingElevenLabs(false);
        return;
      }
      ...
      await audio.play();
      setIsPlayingAudio(true);
      showToast(`Playing AI Edge Neural narration (${selectedVoiceId.includes("Gadis") ? "Female" : "Male"})!`);
    } catch (e: any) {
      clearTimeout(timeoutId);
      if (e?.name === "AbortError") {
        showToast("Edge Neural AI connection timed out (>6s). Falling back to browser voice...");
      } else {
  ...
  const handleReaction = (type: "LIKE" | "DISLIKE") => {
    if (!user) {
      showToast("Please log in first to react");
      router.push(`/login?message=${encodeURIComponent("Please log in first to react")}`);
      return;
    }
    ...
    fetch("/api/reaction", {
      ...
      .catch(() => {
        setArticle((prev) => ({ ...prev, ...previousState }));
        showToast("Failed to process article reaction.");
      });
  };
  ...
  const debouncedAddComment = useDebouncedAction(() => {
    ...
    if (!user) {
      showToast("Please log in first to comment");
      router.push(`/login?message=${encodeURIComponent("Please log in first to comment")}`);
      return;
    }
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() => {
                ...
                showToast(nowSaved ? "Article saved to your profile!" : "Article removed from saved items.");
              }}
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[11px] font-semibold text-slate-800">
                {isPlayingAudio ? "🔊 Reading article..." : `Ready to play (${calculatedReadTime} min · ${wordCount} words)`}
              </p>
            </div>
          </div>
          ...
          {user ? (
            <form onSubmit={handleAddComment} className="p-4 sm:p-5 rounded-none border-2 border-black dark:border-white bg-white dark:bg-black space-y-4 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:shadow-[3px_3px_0px_0px_rgba(255,255,255,1)]">
              <div className="flex items-center gap-2 text-xs font-bold text-black dark:text-white">
                ...
                <span>{replyTargetId ? "REPLYING TO COMMENT" : "POSTING AS"} <strong className="text-[#166534] dark:text-[#EAB308] uppercase">{user.user_metadata?.full_name || user.email?.split("@")[0]}</strong></span>
              </div>

              <textarea
                rows={3}
                placeholder={replyTargetId ? "Write your reply..." : "Share your thoughts..."}
                value={commentText}
